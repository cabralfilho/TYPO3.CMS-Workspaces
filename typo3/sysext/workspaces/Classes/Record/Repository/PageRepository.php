...
class PageRepository implements \TYPO3\CMS\Core\SingletonInterface {
	...
	const PAGES_Fields = 'uid,pid,tstamp,sorting,deleted,hidden,doktype,title,nav_title,t3ver_oid,t3ver_id,t3ver_wsid,t3ver_label,t3ver_state,t3ver_stage,t3ver_count,t3ver_tstamp,t3ver_move_id,t3_origuid,perms_userid,perms_groupid,perms_user,perms_group,perms_everybody,is_siteroot,storage_pid';
	...
	/**
	 * Gets the live version of a record.
	 *
	 * @param integer $pageId Uid of the offline/draft record
	 * @return NULL|array
	 */
	public function getLiveVersion($pageId) {
		$liveVersion = NULL;

		$currentRecord = $this->findById($pageId);
		if ($currentRecord !== NULL && (int) $currentRecord['pid'] === -1) {
			$liveVersionId = $currentRecord['t3ver_oid'];
			$liveVersion = $this->findById($liveVersionId);
		}
	 ...
	 */
	public function getWorkspaceOverlay($pageId, $workspaceId = -99, $unsetMovePointers = FALSE) {
